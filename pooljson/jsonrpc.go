...
// valid JSON type.  JSON-RPC 2.0 only allows string, number, or null, so this
// function restricts the allowed types  to that list.  This function is only
// provided in case the caller is manually marshalling for some reason.
// The functions which accept an ID in this package already call this function
// to ensure the provided id is valid.
...
// SimplifiedRequest is a type for raw JSON-RPC requests.  The Method field identifies
// the specific command type which in turns leads to different parameters.
// Callers typically will not use this directly since this package provides a
// statically typed command infrastructure which handles creation of these
// requests, however this struct it being exported in case the caller wants to
...
// NewRequest2 returns a new JSON-RPC 1.0 request object given the provided id,
...
// type with the NewCmd or New<Foo>Cmd functions and call the MarshalCmdJson
// function with that command to generate the marshalled JSON-RPC request.
// Params are in list form.
func NewRequest2(id interface{}, method string, params []interface{}) (*Request, error) {
	if !IsValidIDType(id) {
...
// type with the NewCmd or New<Foo>Cmd functions and call the MarshalCmdJson
// function with that command to generate the marshalled JSON-RPC request.
// Params are in JSON form.
func NewRequest(id interface{}, method string, params json.RawMessage) (*SimplifiedRequest, error) {
	if !IsValidIDType(id) {
		str := fmt.Sprintf("the id of type '%T' is invalid", id)
		return nil, makeError(ErrInvalidType, str)
	}
...
func MarshalResponse(id interface{}, result interface{}, rpcErr *RPCError) ([]byte, error) {
	...
	// 	return nil, err
	// }
	fmt.Printf("result of response is %v\n", result)

	response, err := NewResponse(id, result, rpcErr)
	if err != nil {
		return nil, err
	}
	fmt.Printf("result of response marshal is %v\n", response)
	res, err := json.Marshal(&response)

	fmt.Printf("result of response marshal is %s\n", res)
	return res, err
}
