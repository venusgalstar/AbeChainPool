...
// Client represents a  RPC client which allows easy access to the
// various RPC methods available on a  RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *abejson.NotificationTransactionsCmd:
		c.ntfnState.notifyTransaction = true
	}
}
...
func (c *Client) reregisterNtfns() error {
	...
	// Reregister notifications if needed.
	...
	if stateCopy.notifyTransaction {
		log.Debugf("Reregistering [notifytransaction]")
		if err := c.NotifyTransactions(); err != nil {
			return err
		}
	}
	return nil
...
out:
	...
	reconnect:
		for {
			...
			if err != nil {
				c.retryCount++
				log.Infof("Failed to connect to %v (%s): %v",
					c.config.Alias, c.config.Host, err)
				...
				log.Infof("Retrying connection to %v (%s) in "+
					"%s", c.config.Alias, c.config.Host, scaledDuration)
				time.Sleep(scaledDuration)
				continue reconnect
			}

			log.Infof("Reestablished connection to %v (%s)",
				c.config.Alias, c.config.Host)
...
func receiveFuture(f chan *response, timeout ...int) ([]byte, error) {
	...
	timeoutTicker := time.NewTicker(time.Duration(timeout[0]) * time.Second)
	defer timeoutTicker.Stop()
	select {
	case r := <-f:
		return r.result, r.err
	case <-timeoutTicker.C:
		return nil, abejson.ErrRPCTimeout
	}
}
...
func (c *Client) sendPost(jReq *jsonRequest) {
	...
	url0 := protocol + "://" + c.config.Host
	bodyReader := bytes.NewReader(jReq.marshalledJSON)
	httpReq, err := http.NewRequest("POST", url0, bodyReader)
	if err != nil {
...
func (c *Client) sendRequest(jReq *jsonRequest) {
	...
	log.Debugf("Sending command [%s] to %v", jReq.method, c.config.Host)
	c.sendMessage(jReq.marshalledJSON)
}
...
type ConnConfig struct {
	...
	// Alias is the name of remote RPC server. (e.g. Abec, Abewallet)
	Alias string
}
...
func dial(config *ConnConfig) (*websocket.Conn, error) {
	...
	// Dial the connection.
	url0 := fmt.Sprintf("%s://%s/%s", scheme, config.Host, config.Endpoint)
	wsConn, resp, err := dialer.Dial(url0, requestHeader)
	if err != nil {
		log.Errorf("Unable to connect to %v (%v)", config.Alias, config.Host)
...
func New(config *ConnConfig, ntfnHandlers *NotificationHandlers) (*Client, error) {
	...
	case chaincfg.RegNetParams.Name:
		client.chainParams = &chaincfg.RegNetParams
	...
	if start {
		log.Infof("Established connection to Abec (%s)",
			config.Host)
...
func (c *Client) Connect(tries int) error {
	...
	for i := 0; tries == 0 || i < tries; i++ {
		...
		if err != nil {
			...
			log.Infof("Reconnect to %v after %v", c.config.Alias, backoff)
			time.Sleep(backoff)
			continue
		}
		...
		// to run the client.
		log.Infof("Established connection to %v (%s)",
			c.config.Alias, c.config.Host)
		c.wsConn = wsConn
